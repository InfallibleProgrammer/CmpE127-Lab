...
const char KeypadReg = 4; //Pass 4 into command register. 00000100.
...
char keypadValue(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        printf("VINCENT!!!!!!!!!!!!: %02x\n", byte & 0xFF);
        if (byte > 15 && byte < 32) return arr[i][0];
        if (byte > 31 && byte < 64) return arr[i][1];
        if (byte > 63 && byte < 128) return arr[i][2];
        if (byte > 127 && byte < 256) return arr[i][3];
    }
}
